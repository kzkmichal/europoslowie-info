...
export const SpeechesList = ({
  ...
  if (speeches.length === 0) {
    return <div className="text-sm text-outline">Brak przemówień</div>
  }
      ...
      {speeches.map((speech) => (
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <p className="flex-1 text-sm font-semibold text-on-surface leading-snug">
              {speech.debateTopic}
            </p>
          ...
          {speech.transcript && (
            <details className="mt-3">
              <summary className="cursor-pointer text-xs font-semibold text-primary hover:text-on-primary-container transition-colors">
